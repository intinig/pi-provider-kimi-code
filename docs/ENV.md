...
| `KIMI_CODE_PROTOCOL`       | Select the wire protocol. Supported values: `openai` (default) or `anthropic`. `openai` → `openai-completions` via `/coding/v1/chat/completions`; `anthropic` → `anthropic-messages` via `/coding/v1/messages`. Any value other than `anthropic` is treated as `openai`; an explicit per-model protocol returned by `/models` takes precedence. |
