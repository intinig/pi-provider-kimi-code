...
These mirror the same-name environment variables in upstream `kimi-cli`. When set they apply *after* server-side discovery, so a user-provided override always wins over `/v1/models` metadata.
